...
public function exportTeachers(Request $request)
{
    ...
    $query = \App\Models\Teacher::with(['user', 'subjects'])
        ->where('school_id', $schoolId);
...
public function exportStaff(Request $request)
{
    ...
    $query = \App\Models\Staff::with('user')
        ->where('school_id', $schoolId);
...
public function exportParents(Request $request)
{
    ...
    $query = \App\Models\Parents::with(['user', 'student.user'])
        ->where('school_id', $schoolId);
   ...
   public function exportExamResults(Request $request)
    {
        ...
        // Optionally apply filters (e.g., academic_year, class, subject)
        $query = ExamResult::with(['exam', 'student'])
            ->whereHas('student', function ($q) use ($schoolId) {
                $q->where('school_id', $schoolId);
            });

        // Apply request-based filters if needed
        ...
        return Excel::download(new ExamResultsExport($results), 'exam_results.xlsx');
    }
